...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 00:20:41 2020
...
with open("month_lof/dhist_for_rand_col3_data.csv", "w") as f05:
    ...
    for filename in all_files:
        ...
        test_data1=[values for values in df.iloc[train_len:, 0:2].values]
        
        test_data2=[values for values in df2.iloc[:, 2:3].values]
    
        test_data3=[values for values in df.iloc[train_len:, 3:columns].values]
        
        ll1=[]
        ll2=[]
        for i in range(len(test_data1)):
            
            ll1.append(test_data1[i][0])
            ll2.append(test_data1[i][1])
        
        ll3=[]
        for sub in test_data2:
            for item in sub:
                ll3.append(item)
                
        ll4=[]
        for sub in test_data3:
            for item in sub:
                ll4.append(item)
        #print(ll1)
